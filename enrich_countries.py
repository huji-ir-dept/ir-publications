...
def main():
    ...
    for i, pub in enumerate(needs_enrichment, 1):
        print(f"  [{i}/{len(needs_enrichment)}] {(pub.get('title') or 'ללא כותרת')[:55]}...", flush=True)
        ...
        for a in authorships:
            ...
            # אם אין country מהמוסד — נסה לחלץ מ-raw_affiliation_strings
            if not country and raw_affiliations:
                # שמור את המוסד הגולמי גם ללא מדינה
                institution = raw_affiliations[0][:120] if raw_affiliations else None
